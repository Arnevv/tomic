...
def toon_details(trade):
    ...
    kolom1_keys = ["TradeID", "DatumIn", "DatumUit", "Symbool", "Type", "Expiry"]
    kolom2_keys = ["Richting", "Status", "Premium", "EntryPrice", "ExitPrice", "ReturnOnMargin"]
    kolom3_keys = ["StopPct", "IV_Entry", "HV_Entry", "DaysInTrade"]

    def print_kolomrij(label, key):
        waarde = trade.get(key, "")
        return f"{label}: {waarde}"

    print("\n📄 Overzicht:")
    for r in range(max(len(kolom1_keys), len(kolom2_keys), len(kolom3_keys))):
        c1 = print_kolomrij(kolom1_keys[r], kolom1_keys[r]) if r < len(kolom1_keys) else ""
        c2 = print_kolomrij(kolom2_keys[r], kolom2_keys[r]) if r < len(kolom2_keys) else ""
        c3 = print_kolomrij(kolom3_keys[r], kolom3_keys[r]) if r < len(kolom3_keys) else ""
    ...
    if "Plan" in trade and "Delta" in trade.get("Greeks", {}):
        print("\n📌 Originele Entry-Greeks uit plan (ter referentie):")
        lines = trade["Plan"].splitlines()
        greek_lines = [l for l in lines if any(g in l for g in ["Delta", "Theta", "Gamma", "Vega"])]
    ...
    def float_input(prompt):
        while True:
            ...
            try:
                ...
                print("❌ Ongeldige invoer. Vul een getal in zoals -4.2 of 12.8.")
...
def pas_trade_aan(trade):
    velden = ["Plan", "Reden", "Exitstrategie", "Status", "DatumUit", "Resultaat", "Opmerkingen", "Evaluatie", "ExitPrice", "ReturnOnMargin"]
    while True:
