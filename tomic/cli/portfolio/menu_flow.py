    ...
    while True:
        ...
        if sel == "998":
            ...
            if refreshed is not None:
                # Check if any rejections were promoted to candidates
                if session.scan_rejections:
                    # Get symbols/strategies that are now candidates
                    candidate_keys = {
                        (c.symbol.upper(), c.strategy.lower().replace(" ", "_"))
                        for c in refreshed
                    }

                    # Find rejections that are now in candidates (promoted)
                    promoted_count = 0
                    remaining_rejections: list[Mapping[str, Any]] = []
                    for entry in session.scan_rejections:
                        symbol = (
                            entry.get("scan_symbol")
                            or entry.get("symbol")
                            or ""
                        ).upper()
                        strategy = (
                            entry.get("scan_strategy")
                            or entry.get("strategy")
                            or ""
                        ).lower().replace(" ", "_")
                        if (symbol, strategy) in candidate_keys:
                            promoted_count += 1
                            logger.info(
                    ...
                    session.scan_rejections = remaining_rejections
                    if promoted_count > 0:
                        print(f"✅ {promoted_count} voorstel(len) gepromoveerd naar kandidaten.")
