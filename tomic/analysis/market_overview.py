...
def build_market_overview(
    ...
    for idx, rec in enumerate(recs, 1):
        ...
        ivr = rec.get("iv_rank")
        # iv_rank is in 0-1 scale from normalize_percent(), multiply by 100 for display
        iv_rank_val = f"{ivr * 100:.0f}" if isinstance(ivr, (int, float)) else ""
        ivp = rec.get("iv_percentile")
        # iv_percentile is in 0-1 scale from normalize_percent(), multiply by 100 for display
        iv_percentile_val = f"{ivp * 100:.0f}" if isinstance(ivp, (int, float)) else ""
